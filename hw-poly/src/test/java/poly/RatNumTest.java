...
public final class RatNumTest {
    ...
    /**
     * ratnums: Set of varied ratnums (includes NaNs) set is { 0, 1, -1, 2, 1/2, 3/2, 1/0, -1/0, 100/0
     * }
     */
     ...
     */
    private RatNum[] ratNaNs = new RatNum[]{one_I_zero, negOne_I_zero, hundred_I_zero};

    /**
     * ratNonNaNs: Set of varied non-NaN ratNums set is ratNums - ratNaNs
     */
    private RatNum[] ratNonNaNs = new RatNum[]{zero, one, negOne, two, one_I_two, three_I_two};
    ...
    public void testDoubleValueNaN() {
        ...
        assertEquals(new Double(Double.NaN), new Double(one_I_zero.doubleValue()));
    }
    ...
    public void testFloatValueNaN() {
        ...
        assertEquals(new Float(Float.NaN), new Float(one_I_zero.floatValue()));
    }
    ...
    public void testIntValueUnsupported() {
        if(false) {
            ...
            assertEquals(
                    "(MIN_VALUE+1)/MIN_VALUE should round to 1",
                    1,
                    new RatNum(Integer.MIN_VALUE + 1, Integer.MIN_VALUE).intValue());
            assertEquals("1/MIN_VALUE should round to 0", 0, new RatNum(1, Integer.MIN_VALUE).intValue());
        }
    }
    ...
    public void testIntValueWithRoundMaxValue() {
        assertEquals(
                "MAX_VALUE should round to MAX_VALUE",
                Integer.MAX_VALUE,
                new RatNum(Integer.MAX_VALUE).intValue());
        assertEquals(
                "MAX_VALUE/2 should round to (MAX_VALUE/2)+1",
                (Integer.MAX_VALUE / 2) + 1,
                new RatNum(Integer.MAX_VALUE, 2).intValue());
    }

    @Test // Ensure 1 / Max Value == 0
    public void testIntValueWithRoundOneOverMaxValue() {
        assertEquals("1/MAX_VALUE should round to 0", 0, new RatNum(1, Integer.MAX_VALUE).intValue());
    }

    @Test // Ensure Max Value / Max Value == 1
    public void testIntValueMaxValueDivByMaxValue() {
        assertEquals(
                "MAX_VALUE/MAX_VALUE should round to 1",
                1,
                new RatNum(Integer.MAX_VALUE, Integer.MAX_VALUE).intValue());
        assertEquals(
                "(MAX_VALUE-1)/MAX_VALUE should round to 1",
                1,
                new RatNum(Integer.MAX_VALUE - 1, Integer.MAX_VALUE).intValue());
    }

    @Test
    public void testIntValueWithRoundMinValue() {
        assertEquals(
                "MIN_VALUE should round to MIN_VALUE",
                Integer.MIN_VALUE,
                new RatNum(Integer.MIN_VALUE).intValue());
        assertEquals(
                "MIN_VALUE/2 should round to (MIN_VALUE/2)",
                (Integer.MIN_VALUE / 2),
                new RatNum(Integer.MIN_VALUE, 2).intValue());
    }

    @Test // Ensure Min Value / Min Value == 1
    public void testIntValueMinValueDivByMinValue() {
        assertEquals(
                "MIN_VALUE/MIN_VALUE should round to 1",
                1,
                new RatNum(Integer.MIN_VALUE, Integer.MIN_VALUE).intValue());
    }
    ...
    private void decChk(String s, RatNum expected) {
        assertTrue(RatNum.valueOf(s).equals(expected));
    }
